...
export default function Dashboard({ reportType = "level-1" }) {
    ...
    const handleDownload = async () => {
        try {
            ...
            link.href = url;
            link.setAttribute("download", `Report_${reportType}_${reportData.client_name}.docx`);
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <header className="flex flex-col md:flex-row justify-between items-center mb-10 gap-4">
                    <div className="flex items-center gap-3">
                        <button
                            onClick={() => navigate("/")}
                            className="bg-indigo-600 p-2 rounded-xl shadow-lg shadow-indigo-500/30 hover:bg-indigo-500 transition-colors"
                            title="Back to Home"
                        >
                            <LayoutDashboard className="w-6 h-6 text-white" />
                        </button>
                        <h1 className="text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-white to-slate-400">
                            ReportMaker<span className="text-indigo-400">.ai</span> <span className="text-sm font-light text-slate-500 ml-2 border border-slate-700 px-2 py-0.5 rounded-full">{reportType === 'closure' ? 'Closure' : 'Level-1'}</span>
                        </h1>
                    </div>
                                ...
                                <div className="space-y-4">
                                    ...
                                    {reportType === 'closure' && (
                                        <div className="grid grid-cols-2 gap-3 animate-fadeIn">
                                            <div>
                                                <label className="text-xs font-semibold text-rose-300 uppercase tracking-wider mb-1 block">Level 1 End Date</label>
                                                <input
                                                    type="date"
                                                    className="w-full glass-input p-3 rounded-lg text-sm border-rose-500/20 focus:border-rose-500/50"
                                                    value={reportData.level_1_end_date}
                                                    onChange={(e) =>
                                                        setReportData({ ...reportData, level_1_end_date: e.target.value })
                                                    }
                                                />
                                            </div>
                                            <div>
                                                <label className="text-xs font-semibold text-emerald-300 uppercase tracking-wider mb-1 block">Level 2 Start Date</label>
                                                <input
                                                    type="date"
                                                    className="w-full glass-input p-3 rounded-lg text-sm border-emerald-500/20 focus:border-emerald-500/50"
                                                    value={reportData.level_2_start_date}
                                                    onChange={(e) =>
                                                        setReportData({ ...reportData, level_2_start_date: e.target.value })
                                                    }
                                                />
                                            </div>
                                        </div>
